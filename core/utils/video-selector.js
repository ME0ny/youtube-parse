...
/**
 * Проверяет, является ли текст, вероятно, русскоязычным.
...
export function isLikelyRussian(text) {
    ...
    // --- 2. Определяем маркеры НЕ-РУССКОГО языка ---
    ...
    const belarusian = 'ўЎ';
    // Казахские буквы
    const kazakh = 'қғңһөүәҚҒҢҺӨҮӘ';
    // Киргизские буквы (кроме тех, что есть в русском)
    const kyrgyz = 'ңүөәҢҮӨӘ';
    // Узбекские буквы
    const uzbek = 'ҳқҲҚ';
    // Таджикские буквы
    const tajik = 'ғӣқӯҳҷҒӢҚӮҲҶ';
    // Сербские/Македонские буквы
    const serbian = 'ђћџЂЋЏѓќѓќЃЌ';

    // 👇 ИСПРАВЛЕНО: Объединяем ВСЕ маркеры в ОДИН набор символов
    const allNonRussianChars = ukrainian + belarusian + kazakh + kyrgyz + uzbek + tajik + serbian;
    const nonRussianMarkers = new RegExp(`[${allNonRussianChars}]`);
    ...
    // --- 3. Умная проверка латинской буквы "i" ---
    // Ищем латинскую "i" или "I", окружённую кириллическими символами (слева ИЛИ справа)
    const cyrillicChar = '[\\u0400-\\u04FF\\u0500-\\u052F]';
    const latinIMarker = new RegExp(`${cyrillicChar}[iI]|[iI]${cyrillicChar}`);

    if (latinIMarker.test(text)) {
        return false;
    }
